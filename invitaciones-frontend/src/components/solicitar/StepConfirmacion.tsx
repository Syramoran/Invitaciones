...
export function StepConfirmacion({ confirmedOrder, eventType, templateName, form, addons, addonsData, secondVersion, prices }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto text-center">
      ...
      <h2 className="font-display text-4xl font-bold mb-3 dark:text-cream">¡Pedido recibido!</h2>
      <p className="text-warm-gray dark:text-cream/80 font-light text-lg mb-10 leading-relaxed">
      ...
      <div className="bg-white dark:bg-charcoal rounded-2xl border border-black/[0.06] dark:border-white/5 p-7 text-left mb-6">
        <h4 className="font-display text-xl font-semibold mb-4 dark:text-cream">Detalle del pedido</h4>
        <span className="inline-block bg-champagne dark:bg-charcoal-soft px-4 py-1.5 rounded-full text-sm font-semibold mb-5 dark:text-cream">{orderId}</span>
      ...
      <div className="bg-ivory dark:bg-charcoal-soft rounded-2xl p-7 text-left mb-8">
        <h4 className="font-display text-xl font-semibold mb-4 dark:text-cream">Próximos pasos</h4>
        <ol className="space-y-3">
          {NEXT_STEPS.map((s, i) => (
            <li key={i} className="flex items-start gap-3 text-sm font-light text-charcoal-soft dark:text-cream/90">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        ...
        <Link
          to="/"
          className="inline-flex items-center justify-center gap-2 border border-charcoal dark:border-cream text-charcoal dark:text-cream px-9 py-4 rounded-full font-medium hover:bg-charcoal hover:text-champagne-light dark:hover:bg-cream dark:hover:text-charcoal transition-all"
        >
          Volver al inicio
        </Link>
      </div>
