...
export function StepConfirmacion({ confirmedOrder, eventType, templateName, form, addons, secondVersion, prices }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto text-center">
      ...
      <h2 className="font-display text-4xl font-bold mb-3">¡Pedido recibido!</h2>
      <p className="text-warm-gray font-light text-lg mb-10 leading-relaxed">
        Nos vamos a contactar con vos por WhatsApp para coordinar los detalles de tu evento y el pago.
      </p>

      {/* Order details */}
      <div className="bg-white rounded-2xl border border-black/[0.06] p-7 text-left mb-6">
        <h4 className="font-display text-xl font-semibold mb-4">Detalle del pedido</h4>
        <span className="inline-block bg-champagne px-4 py-1.5 rounded-full text-sm font-semibold mb-5">{orderId}</span>
        <div className="space-y-3">
          ...
          <div className="border-t border-ivory pt-3 flex justify-between items-center">
      ...
      {/* Next steps */}
      <div className="bg-ivory rounded-2xl p-7 text-left mb-8">
        <h4 className="font-display text-xl font-semibold mb-4">Próximos pasos</h4>
        <ol className="space-y-3">
          {NEXT_STEPS.map((s, i) => (
            <li key={i} className="flex items-start gap-3 text-sm font-light text-charcoal-soft">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        ...
        <Link
          to="/"
          className="inline-flex items-center justify-center gap-2 border border-charcoal text-charcoal px-9 py-4 rounded-full font-medium hover:bg-charcoal hover:text-champagne-light transition-all"
        >
          Volver al inicio
        </Link>
      </div>
