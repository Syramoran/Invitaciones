...
function SingleMapCard({
  ...
  return (
    <div>
      ...
      <p className="mb-1 text-[15px] font-medium text-white">{ubicacion}</p>
      {direccion && <p className="mb-3 text-[13px] text-[#999999]">{direccion}</p>}

      <div className="mb-3 h-[180px] w-full overflow-hidden rounded-xl border border-[#333333]">
        <iframe
          src={embedUrl}
          width="100%"
          height="100%"
      ...
      <div className="flex gap-2.5">
        ...
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-[#444444] bg-transparent px-3 py-2.5 text-xs font-semibold text-[#cccccc] transition-colors hover:bg-white/10"
        >
          {copiado ? (
...
export function MapSection({ ubicacion, direccion, latitud, longitud, camposEspecificos }: MapSectionProps) {
  ...
  return (
    <section className="border-b border-[#2a2a2a] px-7 py-12">
      <p className="mb-6 text-center text-[13px] font-medium tracking-[0.3em] text-[#aaaaaa] uppercase">
        Ubicación
      </p>
